// controllers/parent/parent.fee.controller.js
...
// Helper functions EXACTLY LIKE ADMIN & STUDENT
const safeNumber = (v) => {
...
export const getChildFeeStatus = asyncHandler(async (req, res) => {
  ...
  if (!child) throw new ValidationError('This child does not belong to you');
  ...
  if (!feePayment) {
    return successResponse(res, 'No fee record found for this academic year', {
      student: {
        name: child.name,
  ...
  // Compute next due installment info from installments[]
  const now = new Date();
  ...
  return successResponse(res, 'Child fee status retrieved successfully', {
    ...feePayment.toObject(),
...
// ==========================
// 2. All children fee status
// ==========================
export const getAllChildrenFeeStatus = asyncHandler(async (req, res) => {
  const parentId = req.user.id;
  const academicYear = req.query.academicYear || getCurrentAcademicYear();
  ...
  const parent = await Parent.findOne({
    _id: parentId,
    schoolId: req.schoolId
  }).populate('children', 'name studentID rollNumber className section');
  if (!parent) throw new NotFoundError('Parent');

  const feeStatusPromises = parent.children.map(async (child) => {
    const feePayment = await FeePayment.findOne({
      schoolId: req.schoolId,
      student: child._id,
      academicYear
    ...
    return {
      ...
      feeStatus: {
        ...
        // for old UI compatibility, optional:
        dueDate: feePayment.dueDate || null,
        // new installment-aware info:
        nextInstallment: nextInstallment
          ? {
              name: nextInstallment.name,
              dueDate: nextInstallment.dueDate,
              pendingAmount:
                (nextInstallment.amount || 0) - (nextInstallment.paidAmount || 0),
              status: nextInstallment.status
            }
          : null
      }
  ...
  const feeStatuses = await Promise.all(feeStatusPromises);
  return successResponse(res, 'All children fee status retrieved successfully', feeStatuses);
});

export const getChildPaymentHistory = asyncHandler(async (req, res) => {
  const parentId = req.user.id;
  const { childId } = req.params;
...
export const getFeeReceipt = asyncHandler(async (req, res) => {
  ...
  // ✅ MULTI-TENANT: Get fee payment with school filter
  const feePayment = await FeePayment.findOne({
    schoolId: req.schoolId,  // ✅ MULTI-TENANT
    _id: feePaymentId
  })
  ...
  const parent = await Parent.findOne({
    _id: parentId,
    schoolId: req.schoolId,
    children: feePayment.student._id  // MongoDB $in check
  });
...
// ✅ FIXED: Download fee receipt - CONSISTENT WITH ADMIN & STUDENT
export const downloadFeeReceipt = asyncHandler(async (req, res) => {
  try {
    const parentId = req.user.id;
    const { feePaymentId, paymentId } = req.params;

    console.log("📥 Parent PDF Download START:", { feePaymentId, paymentId, parentId });

    // ✅ MULTI-TENANT: FeePayment + school filter
    const feePayment = await FeePayment.findOne({
      schoolId: req.schoolId,  // ✅ MULTI-TENANT
      _id: feePaymentId,
      "payments._id": paymentId,
    })
      .populate("student", "name studentID rollNumber className section fatherName motherName address phone")
      .populate("payments.receivedBy", "name adminID");

    if (!feePayment) {
      console.log("❌ FeePayment not found for school");
      return res.status(404).end();
    }

    // ✅ MULTI-TENANT: Parent-child ownership check
    const parent = await Parent.findOne({
      _id: parentId,
      schoolId: req.schoolId,
      children: feePayment.student._id
    });
    ...
    // 2) School info (same as admin & student)
    ...
    // 3) Safe numbers - EXACTLY LIKE ADMIN & STUDENT
    ...
    // 4) Headers - PDF download EXACTLY LIKE ADMIN & STUDENT
    res.set({
    ...
    // ✅ FIXED: Use drawAmount helper EXACTLY LIKE ADMIN & STUDENT
    ...
    doc.end();
    console.log("✅ Parent PDF generated for:", studentName, "Receipt:", payment.receiptNumber);
  } catch (error) {
    console.error("💥 Parent PDF ERROR:", error.message);
    if (!res.headersSent) {
